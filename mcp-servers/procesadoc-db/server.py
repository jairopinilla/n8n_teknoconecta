"""MCP server for procesadoc — PostgreSQL management on chitara VPS via SSH.

Usage (from procesadoc repo / developer machine):
  - Requires uv (pip install uv)
  - Requires SSH key at ~/.ssh/id_ed25519 authorized on VPS
  - Set env: PROCESADOC_SSH_HOST, PROCESADOC_SSH_USER, PROCESADOC_DB_USER, PROCESADOC_DB_PASS
"""
...
SSH_HOST = os.environ.get("PROCESADOC_SSH_HOST", "5.252.52.190")
SSH_USER = os.environ.get("PROCESADOC_SSH_USER", "root")
DB_USER = os.environ.get("PROCESADOC_DB_USER", "procesadoc_app")
DB_PASS = os.environ.get("PROCESADOC_DB_PASS", "MepZcR7YaVwEfaS72id-0mnqIeEFVjor")
DB_NAME = os.environ.get("PROCESADOC_DB_NAME", "procesadoc")
...
server = Server("procesadoc-db")

@server.list_tools()
async def list_tools():
    return [
        Tool(
            name="procesadoc_list_schemas",
            description="List all database schemas in procesadoc.",
            inputSchema={"type": "object", "properties": {}, "required": []},
        ),
        Tool(
            name="procesadoc_list_tables",
            description="List tables in a schema from procesadoc.",
            inputSchema={
                "type": "object",
        ...
        Tool(
            name="procesadoc_get_table",
            description="Get table structure (columns, types, constraints) from procesadoc.",
            inputSchema={
                "type": "object",
                "properties": {
        ...
        Tool(
            name="procesadoc_exec_sql",
            description="Execute any SQL statement on procesadoc. Full read-write access.",
            inputSchema={
                "type": "object",
        ...
        Tool(
            name="procesadoc_list_extensions",
            description="List all PostgreSQL extensions from procesadoc.",
            inputSchema={"type": "object", "properties": {}, "required": []},
        ),
        Tool(
            name="procesadoc_list_functions",
            description="List all stored functions in a schema from procesadoc.",
            inputSchema={
                "type": "object",
                "properties": {"schema": {"type": "string", "default": "public"}},
                "required": [],
        ...
        Tool(
            name="procesadoc_get_indexes",
            description="Get indexes for a specific table from procesadoc.",
            inputSchema={
                "type": "object",
                "properties": {
                    "schema": {"type": "string", "default": "public"},
        ...
        Tool(
            name="procesadoc_server_info",
            description="Get procesadoc server info and DB stats.",
            inputSchema={"type": "object", "properties": {}, "required": []},
        ),
        Tool(
            name="procesadoc_list_roles",
            description="List all database roles from procesadoc.",
            inputSchema={"type": "object", "properties": {}, "required": []},
        ),
        Tool(
            name="procesadoc_query",
            description="Run a read-only SQL query against procesadoc via psql.",
            inputSchema={
                "type": "object",
        ...
        Tool(
            name="procesadoc_get_migrations",
            description="List applied migrations from procesadoc (if tracking table exists).",
            inputSchema={"type": "object", "properties": {}, "required": []},
        ),
        Tool(
            name="procesadoc_apply_migration",
            description="Apply a new migration SQL to procesadoc.",
            inputSchema={
                "type": "object",
                "properties": {
...
async def call_tool(name: str, arguments: dict):
    try:
        if name == "procesadoc_list_schemas":
            rows = _psql_json(
        ...
        elif name == "procesadoc_list_tables":
        ...
        elif name == "procesadoc_get_table":
            schema = arguments.get("schema", "public")
        ...
        elif name == "procesadoc_exec_sql":
            r = _psql(arguments["sql"])
            result = r.get("stdout", r.get("error", ""))

        elif name == "procesadoc_list_extensions":
            rows = _psql_json(
        ...
        elif name == "procesadoc_list_functions":
            schema = arguments.get("schema", "public")
            sql = (
        ...
        elif name == "procesadoc_get_indexes":
            schema = arguments.get("schema", "public")
            table = arguments["table"]
            sql = (
        ...
        elif name == "procesadoc_server_info":
        ...
        elif name == "procesadoc_list_roles":
            rows = _psql_json(
        ...
        elif name == "procesadoc_query":
            r = _psql(arguments["query"])
            result = r.get("stdout", r.get("error", ""))

        elif name == "procesadoc_get_migrations":
            sql = (
                "SELECT json_build_object('name', name, 'applied_at', applied_at) "
                "FROM procesadoc_migrations ORDER BY applied_at DESC"
            )
            rows = _psql_json(sql)
            result = json.dumps(rows, indent=2)

        elif name == "procesadoc_apply_migration":
            if not arguments.get("confirmed", False):
                ...
                r = _psql(arguments["sql"])
                _psql(
                    "CREATE TABLE IF NOT EXISTS procesadoc_migrations "
                    "(name text PRIMARY KEY, applied_at timestamptz DEFAULT now())"
                )
                _psql(
                    f"INSERT INTO procesadoc_migrations (name) VALUES ('{arguments['name']}') "
